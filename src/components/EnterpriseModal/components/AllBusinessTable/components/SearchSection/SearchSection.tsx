...
type EnterpriseModalComponentAllBusinessTableComponentSearchSectionComponentProps = Readonly<{
  enterprise: EnterpriseResponseDto;
  contactId: string | undefined;
  setContactId: (contactId: string | undefined) => void;
}>;
export default function EnterpriseModalComponentAllBusinessTableComponentSearchSectionComponent({
  enterprise,
  contactId,
  setContactId,
}: EnterpriseModalComponentAllBusinessTableComponentSearchSectionComponentProps) {
  // const navigate = routeApi.useNavigate();

  // const { enterpriseId } = routeApi.useParams();
  // const { allBusinessProfileId } = routeApi.useSearch();

  const { data } = useQuery(queries.profiles.list._ctx.byEnterpriseId(enterprise.id));
  ...
  useEffect(() => {
    reset({
      contact: data?.find((item) => item.id === contactId) ?? null,
    });
  }, [contactId, data]);
