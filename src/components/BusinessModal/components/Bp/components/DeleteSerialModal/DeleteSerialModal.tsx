...
type BusinessModalComponentBpComponentDeleteSerialModalComponentProps = Readonly<{
  business: BusinessResponseDto;
  bp: BusinessBpResponseDto;
  serialNumber: BusinessBpSerialResponseDto;
  onClose: () => void;
}>;
export default function BusinessModalComponentBpComponentDeleteSerialModalComponent({
  business,
  bp,
  serialNumber,
  onClose,
}: BusinessModalComponentBpComponentDeleteSerialModalComponentProps) {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.removeQueries({ queryKey: queries['business-bp-serials'].detail._ctx.byId(serialNumber.id).queryKey, exact: true });
