...
type BusinessModalComponentBpComponentDeleteDetailModalComponentProps = Readonly<{
  business: BusinessResponseDto;
  detail: BusinessBpDetailsResponseDto;
  onClose: () => void;
}>;
export default function BusinessModalComponentBpComponentDeleteDetailModalComponent({
  business,
  detail,
  onClose,
}: BusinessModalComponentBpComponentDeleteDetailModalComponentProps) {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => {
      queryClient.removeQueries({ queryKey: queries['business-bp-details'].detail._ctx.byId(detail.id).queryKey, exact: true });
      queryClient.setQueryData<BusinessBpResponseDto>(queries['business-bps'].detail._ctx.byBusinessId(business.id).queryKey, (old) =>
        old
          ? {
              ...old,
              bpDetailsList: old.bpDetailsList.filter((bpDetail) => bpDetail.id !== detail.id),
            }
          : old,
      );
