...
type GedComponentImportFilesModalComponentProps = Readonly<{
  directory: FileDataTreeResponseDto | undefined;
  id: string;
  type: FileType;
  onClose: () => void;
}>;
export default function GedComponentImportFilesModalComponent({ directory, id, type, onClose }: GedComponentImportFilesModalComponentProps) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: () =>
      uploadFilesOnS3(
        directory?.relativePath ?? `ged/${type.toLowerCase()}/${id}/`,
        id,
        type,
        files.map((file) => file.file),
      ),
    onMutate: () => ({ directory, id, type, files }),
