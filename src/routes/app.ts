...
export const Route = createFileRoute('/app')({
  ...
  beforeLoad: ({ search }) => {
    ...
    if (search.appModal === 'create-business' && (search.businessId || search.gedItemKey))
      throw redirect({
        from: Route.id,
        search: (old) => ({ ...old, businessId: undefined, gedItemKey: undefined }),
      });
    if (search.appModal?.startsWith('business-ged') && !search.businessId) {
      toast.error('Aucune affaire sélectionnée');
      throw redirect({
        from: Route.id,
        search: (old) => ({ ...old, modal: undefined, businessId: undefined, gedItemKey: undefined }),
      });
    }
    if (search.appModal === 'business-ged' && search.gedItemKey) {
      throw redirect({
        from: Route.id,
        search: (old) => ({ ...old, gedItemKey: undefined }),
      });
    }
    if ((search.appModal === 'business-ged-rename' || search.appModal === 'business-ged-delete') && !search.gedItemKey)
      throw redirect({
        from: Route.id,
        search: (old) => ({ ...old, modal: 'business-ged', gedItemKey: undefined }),
      });

    if (search.appModal === 'create-client-business' && (search.businessId || search.gedItemKey))
      throw redirect({
        from: Route.id,
        search: (old) => ({ ...old, businessId: undefined, gedItemKey: undefined }),
      });
