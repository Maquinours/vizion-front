...
export const Route = createFileRoute(
  ...
  loader: async ({
    context: { queryClient },
    params: { businessId },
    deps: { lifesheetSize, lifesheetPage, tasksPage, tasksSize },
  }) => {
    ...
    const profileIds = _.uniq(
      [business.createdBy, business.modifiedBy].filter(
        (id) => id !== null,
      ) as Array<string>,
    )
    if (profileIds.length > 0)
      queryClient.prefetchQuery(queries.profiles.list._ctx.byIds(profileIds))
  },
})
