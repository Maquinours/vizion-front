...
export const Route = createFileRoute(
  ...
  loaderDeps: ({
    search: { hideAddresses, hideReferences, hidePrices, hideTotal },
  }) => ({ hideAddresses, hideReferences, hidePrices, hideTotal }),
  loader: async ({
    context: { queryClient },
    params: { businessId },
    deps: { hideAddresses, hideReferences, hidePrices, hideTotal },
  }) => {
    const quotationPromise = queryClient.ensureQueryData(
      queries['business-quotations'].detail._ctx.byBusinessId(businessId),
    )
    ...
    const commercialNoticeData = await queryClient.ensureQueryData(
      queries['commercial-notices'].data._ctx.byProductReferences(
        (quotation.subQuotationList
          ?.flatMap((subQuotation) =>
            subQuotation.quotationDetails?.map(
              (detail) => detail.productReference,
            ),
          )
          ?.filter((reference) => reference !== null) ?? []) as string[],
      ),
    )
    const commercialNoticeBlob = await (
      await fetch(`data:application/pdf;base64,${commercialNoticeData}`)
    ).blob()
    const commercialNoticeFile = new File(
      [commercialNoticeBlob],
      'DOC_COMMERCIAL_VIZEO.pdf',
      { type: commercialNoticeBlob.type },
    )
    ...
    const quotationPdfFile = new File(
      [quotationPdfBlob],
      formatFileName(`Devis-${quotation.number}.pdf`),
      {
