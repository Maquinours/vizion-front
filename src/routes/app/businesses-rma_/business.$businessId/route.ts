...
const searchSchema = z.object({
  businessModal: z
    .enum(['archive', 'assistances', 'create-assistance', 'before-close'])
    .optional()
    .catch(undefined),
})
...
export const Route = createFileRoute(
  ...
  loader: async ({
    ...
    if (
      ((businessModal === 'assistances' ||
        businessModal === 'create-assistance') &&
        (!user.userInfo.roles.includes('ROLE_MEMBRE_VIZEO') ||
          business.state === BusinessState.ARCHIVE)) ||
      (businessModal === 'archive' &&
    ...
    } else if (businessModal === 'create-assistance')
