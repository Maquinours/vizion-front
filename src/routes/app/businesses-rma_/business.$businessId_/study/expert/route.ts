...
export const Route = createFileRoute('/app/businesses-rma_/business/$businessId_/study/expert')({
  loader: async ({ context: { queryClient }, params: { businessId } }) => {
    const user = await queryClient.ensureQueryData(queries.user.authentified());
    if (!user.profile.expert) throw redirect({ from: Route.fullPath, to: '../automatic', replace: true });
    await Promise.all([
