...
export const Route = createFileRoute('/app/external-links_/$externalLinkId')({
    ...
    await queryClient.ensureQueryData({
      ...
      initialData: () => {
        for (const [key, value] of queryClient.getQueriesData<
          Page<ExternalLinkResponseDto>
        >({ queryKey: externalLinks.page._def })) {
  ...
  staticData: {
    getTitle: (queryClient, match) =>
      queryClient
        .ensureQueryData(
          queries['external-link'].detail._ctx.byId(
            (match.params as { externalLinkId: string }).externalLinkId,
          ),
        )
