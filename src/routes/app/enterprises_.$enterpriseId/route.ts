...
export const Route = createFileRoute('/app/enterprises_/$enterpriseId')({
  ...
  loaderDeps: ({
    search: { allBusinessPage, contactsSearch, contactsPage, lifesheetPage },
  }) => ({
    allBusinessPage,
    contactsSearch,
    contactsPage,
  ...
  loader: async ({
    context: { queryClient },
    params: { enterpriseId },
    deps: { allBusinessPage, contactsSearch, contactsPage, lifesheetPage },
  }) => {
    ...
    const enterprisePromise = queryClient.ensureQueryData({
      ...
      initialData: () => {
        for (const [key, value] of queryClient.getQueriesData<
          Page<EnterpriseResponseDto>
        >({ queryKey: enterprises.page._def })) {
    ...
    queryClient.prefetchQuery(
      queries.profiles.page._ctx.byEnterpriseIdAndSearch(
        enterpriseId,
        contactsSearch,
        { page: contactsPage, size: contactsSize },
      ),
    )
    queryClient.prefetchQuery(
      lifesheets
    ...
    queryClient.prefetchQuery(
      geds.detail._ctx.byTypeAndId(FileType.CONTACT, enterpriseId),
    )
  ...
  staticData: {
    getTitle: (queryClient, match) =>
      queryClient
        .ensureQueryData(
          queries.enterprise.detail(
            (match.params as { enterpriseId: string }).enterpriseId,
          ),
        )
        .then((enterprise) => enterprise.name),
  },
})
