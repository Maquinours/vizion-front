...
export const Route = createFileRoute('/app/products_/$productId/informations')({
  validateSearch: (data: { lifesheetPage?: number } & SearchSchemaInput) =>
  ...
  loader: ({
    context: { queryClient },
    params: { productId },
    deps: { lifesheetPage, lifesheetSize, workloadsPage, workloadsSize },
  }) => {
    queryClient.ensureQueryData(users.authentified()).then((user) => {
      if (user.userInfo.roles.includes('ROLE_MEMBRE_VIZEO')) {
        queryClient.prefetchQuery(
          geds.detail._ctx.byTypeAndId(FileType.PRODUIT, productId),
        )
