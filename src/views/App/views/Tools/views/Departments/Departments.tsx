  ...
  columnHelper.display({
    ...
    cell: ({ row: { original } }) => (
      <div className={styles.action_buttons}>
        <Link from={routeApi.id} to="update/$departmentId" params={{ departmentId: original.id }} search={(old) => old} replace>
          <HiPencilAlt width="18" height="18" color="#31385A" />
        </Link>
        <Link from={routeApi.id} to="delete/$departmentId" params={{ departmentId: original.id }} search={(old) => old} replace>
