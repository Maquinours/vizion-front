...
const columns = [
  ...
  columnHelper.display({
    ...
    cell: ({ row: { original } }) => (
      <div>
        <Link from={routeApi.id} to={'./update/$predefinedMessageId'} params={{ predefinedMessageId: original.id }} search={(old) => old} replace>
          <BiEdit size={25} color="#31385A" />
        </Link>
        <Link from={routeApi.id} to={'./delete/$predefinedMessageId'} params={{ predefinedMessageId: original.id }} search={(old) => old} replace>
