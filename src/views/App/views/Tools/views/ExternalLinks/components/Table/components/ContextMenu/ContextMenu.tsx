          ...
          <Fade {...TransitionProps}>
            <Paper className={styles.menu_container}>
              {externalLink && (
                <MenuList>
                  <MenuItem>
                    <Link from={routeApi.id} to="./update/$externalLinkId" params={{ externalLinkId: externalLink.id }} search={(old) => old}>
                      <HiPencilAlt className={styles.icon} />
                      <span className={styles.text}>Modifier</span>
                    </Link>
                  </MenuItem>
                  <MenuItem>
                    <Link from={routeApi.id} to="./archive/$externalLinkId" params={{ externalLinkId: externalLink.id }} search={(old) => old}>
                      <FaFileImport className={styles.icon} />
                      <span className={styles.text}>{externalLink.archived ? 'Désarchiver' : 'Archiver'}</span>
                    </Link>
                  </MenuItem>
                  <MenuItem>
                    <Link from={routeApi.id} to="./delete/$externalLinkId" params={{ externalLinkId: externalLink.id }} search={(old) => old}>
