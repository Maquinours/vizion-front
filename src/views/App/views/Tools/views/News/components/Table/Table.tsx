  ...
  columnHelper.display({
    ...
    cell: ({ row: { original } }) => (
      <div className={styles.action_buttons}>
        <Link from={routeApi.id} to="./update/$newsId" params={{ newsId: original.id }} search={(old) => old} replace>
          <BiEdit size={25} color="#31385A" />
        </Link>
        <Link from={routeApi.id} to="./delete/$newsId" params={{ newsId: original.id }} search={(old) => old} replace>
