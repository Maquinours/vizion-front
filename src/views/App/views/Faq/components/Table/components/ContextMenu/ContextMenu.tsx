              ...
              {faq && (
                <MenuList>
                  {user.userInfo.roles.includes('ROLE_MEMBRE_VIZEO') && [
                    <MenuItem key={0}>
                      <Link from={routeApi.id} to="./update/$faqId" params={(old) => ({ ...old, faqId: faq.id })} search={(old) => old}>
                        <HiPencilAlt className={styles.icon} width={16} height={16} color="#16204E" />
                        <span className={styles.text}>Modifier</span>
                      </Link>
                    </MenuItem>,
                    <MenuItem key={1}>
                      <Link from={routeApi.id} to="./ged/$faqId" params={(old) => ({ ...old, faqId: faq.id })} search={(old) => old}>
                  ...
                  <MenuItem>
                    <Link from={routeApi.id} to="./send-by-email/$faqId" params={(old) => ({ ...old, faqId: faq.id })} search={(old) => old}>
                  ...
                  {user.userInfo.roles.includes('ROLE_MEMBRE_VIZEO') && [
                    <MenuItem key={0}>
                      <Link from={routeApi.id} to="./archive/$faqId" params={(old) => ({ ...old, faqId: faq.id })} search={(old) => old}>
                    ...
                    <MenuItem key={1}>
                      <Link from={routeApi.id} to="./delete/$faqId" params={(old) => ({ ...old, faqId: faq.id })} search={(old) => old}>
