  ...
  return (
    <div className={styles.buttons_container}>
      {user.userInfo.roles.includes('ROLE_REPRESENTANT') && (
        <Link from={routeApi.id} to="representative-turnover" search={(old) => old} replace resetScroll={false} className="btn btn-primary">
          {"Mon chiffre d'affaires"}
        </Link>
      )}
      <Link from={routeApi.id} to="search-by-products" search={(old) => old} replace resetScroll={false} className="btn btn-primary">
