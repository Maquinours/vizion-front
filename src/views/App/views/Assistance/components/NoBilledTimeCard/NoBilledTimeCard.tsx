...
type AppViewAssistanceViewNoBilledTimeCardComponentProps = Readonly<{
  assistance: TechnicalSupportResponseDto;
}>;
export default function AppViewAssistanceViewNoBilledTimeCardComponent({ assistance }: AppViewAssistanceViewNoBilledTimeCardComponentProps) {
  ...
  const startTimer = () => {
    setRunningIntervalId(
      setInterval(() => {
        setValue('noBilledTime', getValues('noBilledTime') + 1);
      }, 1000),
    );
  };
  ...
  const startRunning = () => {
    ...
    window.sessionStorage.setItem(
      ...
      JSON.stringify({ start: Date.now(), value: getValues('noBilledTime') }),
    );
    toast.success('Le chronomètre de SAV a bien été démarré.');
  };

  const stopRunning = () => {
    if (runningIntervalId) clearInterval(runningIntervalId);
    setRunningIntervalId(undefined);
    window.sessionStorage.removeItem(`assistance.${assistance.id}.running_timer.noBilledTime`);
    toast.success('Le chronomètre de paramètrage a bien été arrêté.');
    update();
  };

  useEffect(() => {
    const runningTimer = window.sessionStorage.getItem(`assistance.${assistance.id}.running_timer.noBilledTime`);
    if (runningTimer) {
      ...
      setValue('noBilledTime', Math.floor(duration.as('seconds')));
      startTimer();
    } else setValue('noBilledTime', moment.duration(assistance.noBilledTime).as('seconds'));
  }, [assistance.id]);
  ...
  return (
    <CardComponent title="Temps passé cumulé SAV (non facturé)" className={styles.card}>
      <div className={styles.container}>
        ...
        <div className={styles.buttons_container}>
          <Link from="/app/businesses-rma/business/$businessId/assistance/$assistanceId" to="edit-no-billed-time" replace resetScroll={false}>
