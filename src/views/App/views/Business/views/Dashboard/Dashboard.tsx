      ...
      <div className={styles.container}>
        <div className={styles.headers_buttons}>
          {(business.state === BusinessState.DEVIS || business.state === BusinessState.CREATED) && !business.archived && (
            <Link from={routeApi.id} to="delete" search={(old) => old} className="btn btn-secondary">
          ...
          {!business.archived && (
            <div className={styles.right_buttons}>
              {user.userInfo.roles.includes('ROLE_MEMBRE_VIZEO') && (
                <Link from={routeApi.id} to="send-email" search={(old) => old} className="btn btn-primary">
                  Envoyer un mail
                </Link>
              )}
              {(business.state === null ||
              ...
              <div>
                ...
                <div>
                  {user.userInfo.roles.includes('ROLE_DIRECTION_VIZEO') && (
                    <Link from={routeApi.id} to="update-representative" search={(old) => old} className="btn btn-primary" style={{ marginRight: '10px' }}>
                      Modifier le représentant
                    </Link>
                  )}
                  <Link from={routeApi.id} to="address-book" search={(old) => old} className="btn btn-primary">
                    Carnet d&apos;adresse
                  </Link>
                </div>
              </div>
