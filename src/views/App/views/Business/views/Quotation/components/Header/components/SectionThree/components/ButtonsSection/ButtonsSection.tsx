  ...
  return (
    <div className={styles.actions_container}>
      {!user.userInfo.roles.includes('ROLE_CLIENT') && (
        <>
          <Link from={routeApi.id} search={(prev) => ({ ...prev, hideTotal: !hideTotal })} className="btn btn-primary-light" preload={false} replace>
            {hideTotal ? 'Afficher' : 'Masquer'} total
          </Link>
          <Link from={routeApi.id} search={(prev) => ({ ...prev, hideReferences: !hideReferences })} className="btn btn-primary-light" preload={false} replace>
            {hideReferences ? 'Afficher' : 'Masquer'} les références
          </Link>
          <Link from={routeApi.id} search={(prev) => ({ ...prev, hidePrices: !hidePrices })} className="btn btn-primary-light" preload={false} replace>
            {hidePrices ? 'Afficher' : 'Masquer'} les prix
          </Link>
          <Link from={routeApi.id} search={(prev) => ({ ...prev, hideAddresses: !hideAddresses })} className="btn btn-primary-light" preload={false} replace>
