...
type AppViewStudyViewAutomaticViewIndependantCameraNodeData = {
  model: {
    camId: string;
    reference: string;
  };
...
export default function AppViewStudyViewAutomaticViewIndependantCameraNode({ id, data }: NodeProps<AppViewStudyViewAutomaticViewIndependantCameraNodeData>) {
  ...
  const { data: product } = useQuery({
    ...
    select: (products) => products.find((product) => product.id === data.model.camId),
  });
          ...
          (option) =>
            !!option.qty && (
              <div key={option.reference}>
                ...
                <img src={`https://bd.vizeo.eu/6-Photos/${option.reference}/${option.reference}.jpg`} className="h-8 w-8 opacity-60" />
              </div>
            ),
        )}
      </div>
      <div className="px-1 py-1 hover:relative hover:rounded-md hover:border-2 hover:border-blue-900">
        <img src={`https://bd.vizeo.eu/6-Photos/${data.model.reference}/${data.model.reference}.jpg`} width={'40px'} height={'40px'} onClick={onImageClick} />
      </div>
      <div
        style={{
          display: isSettingsOpened ? 'block' : 'none',
        }}
        className="top-100% absolute z-50 mt-1 h-fit w-[25rem] cursor-default rounded-md border-2 border-[#1a192b] bg-slate-50 px-2"
      >
        ...
        <div>
          <div className="flex items-center justify-start space-x-2 border-b-2 border-b-[#1a192b] p-2">
            <p>Nom du produit</p>
            <input type="text" value={data.name} onChange={onNodeNameChange} placeholder="TV 1" className="rounded-md border border-[#1a192b] p-2" />
          </div>
          <div className="border-b-2 border-b-[#1a192b] px-2">
            ...
            <div className="flex flex-col space-y-3">
              {product?.associatedProduct?.map((option) => {
                ...
                return (
                  <div key={option.id} className="flex items-center justify-start space-x-4">
                    <img src={`https://bd.vizeo.eu/6-Photos/${option.reference}/${option.reference}.jpg`} className="h-12 w-12 object-center" />
