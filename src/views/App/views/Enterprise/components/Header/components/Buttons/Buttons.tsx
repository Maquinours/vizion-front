...
export default function AppViewEnterpriseViewHeaderComponentButtonsComponent() {
  ...
  return (
    <div className={styles.container}>
      <Link from={Route.id} to="./address-book" search={(old) => ({ ...old, search: undefined, page: 0, size: 9 })} className="btn btn-primary">
        {"Carnet d'adresse"}
      </Link>
      {currentUser.userInfo.roles.some((role) => ['ROLE_MEMBRE_VIZEO', 'ROLE_REPRESENTANT'].includes(role)) && (
        <Link from={Route.id} to="./delete" search={(old) => old} className="btn btn-secondary">
