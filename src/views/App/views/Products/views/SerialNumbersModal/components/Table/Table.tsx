    ...
    cell: ({ row: { original } }) => (
      <div style={{ display: 'flex' }}>
        {!original.businessId && !original.businessNumber && (
          <Link from={routeApi.id} to="delete/$serialNumberId" params={{ serialNumberId: original.id }} search={(old) => old}>
            <IoMdTrash size={20} />
          </Link>
        )}
        {original.businessNumber?.startsWith('VZO ') && (
          <Link from={routeApi.id} to="create-rma/$serialNumberId" params={{ serialNumberId: original.id }} search={(old) => old}>
